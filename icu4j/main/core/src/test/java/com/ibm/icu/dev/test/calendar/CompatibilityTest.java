...
public class CompatibilityTest extends CoreTestFmwk {
        ...
        */
        ...
        for (int i = -7; i <= 7; ++i, ++j) {
            ...
            int y = cal.get(Calendar.YEAR),
                    mon = cal.get(Calendar.MONTH) + 1 - Calendar.JANUARY,
    ...
    /**
     * Test the mapping between millis and fields. For the purposes of this test, we don't care
     * about timezones and week data (first day of week, minimal days in first week).
     */
    @Test
    ...
    public void TestGenericAPI() {
        ...
        */
        ...
        // gc2 = gc;
        // if (gc2 != gc || !(gc2 == gc)) errln("FAIL: GregorianCalendar
        // assignment/operator==/operator!= failed");
        // delete gc;
        // delete z;
    }
    ...
    public void TestClonesUnique908() {
        ...
        if (c.get(Calendar.MILLISECOND) != 123 || d.get(Calendar.MILLISECOND) != 456) {
    ...
    void verify765(String msg, IllegalArgumentException e, Calendar c) {
        if (e == null) errln("FAIL: No IllegalArgumentException for " + msg + c.getTime());
    ...
    void aux4064654(int yr, int mo, int dt, int hr, int mn, int sc) {
        ...
        int utc =
                ((cal.get(Calendar.HOUR_OF_DAY) * 60 + cal.get(Calendar.MINUTE)) * 60
    ...
    public void TestAddSetOrder621() {
        ...
        if (s.equals(s2)) logln("Pass: " + s + " == " + s2);
        else errln("FAIL: " + s + " != " + s2);
    }
    ...
    void check520(Calendar c, int y, int m, int d) {
        if (c.get(Calendar.YEAR) != y || c.get(Calendar.MONTH) != m || c.get(Calendar.DATE) != d) {
            errln(
                    "FAILURE: Expected YEAR/MONTH/DATE of "
                            + y
                            + "/"
                            + (m + 1)
                            + "/"
    ...
    public void TestAddAcrossOffsetTransitions() {
        ...
        final TransitionItem[] transitionItems = {
            new TransitionItem(
                    ...
                    Calendar.DECEMBER,
                    8,
                    10), // day before change in ZONE_OFFSET
            new TransitionItem(
                    "US/Pacific", 2011, Calendar.MARCH, 12, 10), // day before change in DST_OFFSET
        };
        for (TransitionItem transitionItem : transitionItems) {
            ...
            cal.set(
                    transitionItem.getYear(),
                    transitionItem.getMonth(),
                    transitionItem.getDay(),
                    itemHour,
                    0);
            ...
            if (hr != itemHour) {
                errln(
                        "Error: Calendar.add produced wrong hour "
                                + hr
                                + " when adding day across transition for zone "
                                + zoneName);
            } else {
                cal.add(Calendar.DATE, -1);
                ...
                if (hr != itemHour) {
                    errln(
                            "Error: Calendar.add produced wrong hour "
                                    + hr
                                    + " when subtracting day across transition for zone "
                                    + zoneName);
                }
            }
        }
    ...
    public void TestAddSetGet0610() {
        ...
        //
        {
            ...
            logln("1B) " + v);
            logln("--) 1993/0/5");
            if (!v.equals(EXPECTED_0610)) errln("Expected " + EXPECTED_0610 + "; saw " + v);
        }
    ...
    public void TestEpochStartFields() {
        ...
        if (z.inDaylightTime(d)) {
            ...
            if (c.get(Calendar.ZONE_OFFSET) != z.getRawOffset())
                errln(
                        "Expected field ZONE_OFFSET to have value "
                                + z.getRawOffset()
                                + "; saw "
            ...
            if (c.get(Calendar.DST_OFFSET) != 0)
                errln(
                        "Expected field DST_OFFSET to have value 0"
                                + "; saw "
    ...
    public void TestActualMinMax() {
        ...
        if (cal.getActualMaximum(Calendar.WEEK_OF_YEAR) != 53)
            errln(
                    "Number of weeks in 1976 should have been 53; got "
                            + cal.getActualMaximum(Calendar.WEEK_OF_YEAR));
    }
    ...
    public void TestRoll() {
        ...
        for (int i = 0; i < dayValues.length; i++) {
            ...
            if (cal2.get(Calendar.DAY_OF_MONTH) != dayValues[i])
                errln(
                        "Rolling the month in 1/31/1997 up by "
                                + i
                                + " should have yielded "
                                + ((i + 1) % 12)
                                + "/"
                                + dayValues[i]
                                + "/1997, but actually yielded "
                                + ((i + 1) % 12)
                                + "/"
        ...
        for (int i = 0; i < dayValues2.length; i++) {
            ...
            if (cal2.get(Calendar.DAY_OF_MONTH) != dayValues2[i]
                    || cal2.get(Calendar.MONTH) != monthValues[i])
                errln(
                        "Roll 2/29/1996 by "
                                + i
                                + " year: expected "
