...
public class TimeZoneRegressionTest extends CoreTestFmwk {
    ...
    public void Test4073215() {
        ...
        if (z.useDaylightTime()) errln("Fail: Fix test to start with non-DST zone");
        z.setStartRule(Calendar.FEBRUARY, 1, Calendar.SUNDAY, 0);
        z.setEndRule(Calendar.MARCH, -1, Calendar.SUNDAY, 0);
        if (!z.useDaylightTime()) errln("Fail: DST not active");
    ...
    /**
     * The expected behavior of TimeZone around the boundaries is: (Assume transition time of 2:00
     * AM) day of onset 1:59 AM STD = display name 1:59 AM ST 2:00 AM STD = display name 3:00 AM DT
     * day of end 0:59 AM STD = display name 1:59 AM DT 1:00 AM STD = display name 1:00 AM ST
     */
    @Test
    public void Test4084933() {
        ...
        long offset1 =
                tz.getOffset(1, 1997, Calendar.OCTOBER, 26, Calendar.SUNDAY, (2 * 60 * 60 * 1000));
        long offset2 =
                tz.getOffset(
                        1, 1997, Calendar.OCTOBER, 26, Calendar.SUNDAY, (2 * 60 * 60 * 1000) - 1);

        long offset3 =
                tz.getOffset(1, 1997, Calendar.OCTOBER, 26, Calendar.SUNDAY, (1 * 60 * 60 * 1000));
        long offset4 =
                tz.getOffset(
                        1, 1997, Calendar.OCTOBER, 26, Calendar.SUNDAY, (1 * 60 * 60 * 1000) - 1);
        ...
        long offset5 =
                tz.getOffset(1, 1997, Calendar.APRIL, 6, Calendar.SUNDAY, (2 * 60 * 60 * 1000));
        long offset6 =
                tz.getOffset(1, 1997, Calendar.APRIL, 6, Calendar.SUNDAY, (2 * 60 * 60 * 1000) - 1);

        long offset7 =
                tz.getOffset(1, 1997, Calendar.APRIL, 6, Calendar.SUNDAY, (1 * 60 * 60 * 1000));
        long offset8 =
                tz.getOffset(1, 1997, Calendar.APRIL, 6, Calendar.SUNDAY, (1 * 60 * 60 * 1000) - 1);
        ...
        if (offset1 != SToffset
                ...
                || offset6 != SToffset
                || offset7 != SToffset
                || offset8 != SToffset) warnln("Fail: TimeZone misbehaving");
    }
        ...
        try {
            for (int i = 0; i < ZONES.length; ++i) {
                ...
                if (!zone.equals(frankenZone)) {
                    logln("TimeZone " + zone.getID() + " not equal to serialized/deserialized one");
                    pass = false;
                }
            }
    ...
    public void Test4109314() {
        ...
        for (int i = 0; i < testData.length; i += 3) {
            ...
            while (t < end.getTime()) {
                ...
                if (!checkCalendar314(testCal, (TimeZone) testData[i])) pass = false;
                t += 60 * 60 * 1000L;
            }
        }
    ...
    boolean checkCalendar314(GregorianCalendar testCal, TimeZone testTZ) {
        ...
        // time.
        int millis =
                testCal.get(Calendar.MILLISECOND)
                        + 1000
                                * (testCal.get(Calendar.SECOND)
                                        + 60
                                                * (testCal.get(Calendar.MINUTE)
     ...
     * CANNOT REPRODUDE
     *
     * <p>Yet another _alleged_ bug in TimeZone.getOffset(), a method that never should have been
     * made public. It's simply too hard to use correctly.
     *
     * <p>The original test code failed to do the following: (1) Call Calendar.setTime() before
     * getting the fields! (2) Use the right millis (as usual) for getOffset(); they were passing in
     * the MILLIS field, instead of the STANDARD MILLIS IN DAY. When you fix these two problems, the
    ...
    public void Test4126678() {
        ...
        if (!tz.inDaylightTime(dt)) errln("We're not in Daylight Savings Time and we should be.\n");
        ...
        int millis =
                cal.get(Calendar.MILLISECOND)
                        + (cal.get(Calendar.SECOND)
                                + (cal.get(Calendar.MINUTE) + (cal.get(Calendar.HOUR) * 60) * 60)
                                        * 1000)
                        - cal.get(Calendar.DST_OFFSET);

        long offset = tz.getOffset(era, year, month, day, dayOfWeek, millis);
        long raw_offset = tz.getRawOffset();
        if (offset == raw_offset) errln("Offsets should not match when in DST");
    ...
    public void Test4151406() {
        ...
        for (int h = -28; h <= 30; ++h) {
            ...
            try {
                ...
                logln(hname + ' ' + ids.length + ((ids.length > 0) ? (" e.g. " + ids[0]) : ""));
            } catch (Exception e) {
                errln(hname + ' ' + "Fail: " + e);
            }
        }
    ...
    public void Test4151429() {
        try {
            ...
            /*String name =*/ tz.getDisplayName(true, Integer.MAX_VALUE, Locale.getDefault());
    ...
    /**
     * SimpleTimeZone accepts illegal DST savings values. These values must be non-zero. There is no
     * upper limit at this time.
     */
    @Test
    public void Test4154525() {
        ...
        int[] DATA = {
            ...
            BAD,
            -1,
            GOOD, // #13566 updates SimpleTimeZone to support negative DST saving amount
            60 * 60 * 1000,
            GOOD,
            Integer.MAX_VALUE,
            GOOD, // no upper limit on DST savings at this time
            Integer.MIN_VALUE,
            GOOD, // no lower limit as well
        };
    ...
    /**
     * SimpleTimeZone.hasSameRules() doesn't work for zones with no DST and different DST
     * parameters.
     */
    @Test
    ...
    /**
     * SimpleTimeZone constructors, setStartRule(), and setEndRule() don't check for out-of-range
     * arguments.
     */
    @Test
    public void Test4154542() {
        ...
        for (int i = 0; i < DATA.length; i += 5) {
            ...
            ex = null;
            try {
                /*SimpleTimeZone temp =*/ new SimpleTimeZone(
                        0,
                        "Z",
                        month,
                        day,
    ...
    /**
     * TimeZone broken at midnight. The TimeZone code fails to handle transitions at midnight
     * correctly.
     */
    @Test
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                logln(
                        "h="
                                ...
                                + dom
                                + "; ZONE_OFFSET="
                                + cal.get(Calendar.ZONE_OFFSET) / H
                                + "; DST_OFFSET="
                                + dstOffset / H
                                + "; getOffset()="
                                + off / H
    ...
    public void Test4173604() {
        ...
        if (o22 != o23 || o22 != o00) {
            errln(
                    "Offsets should be the same (for PST), but got: "
                            + "12/31 22:00 "
                            + o22
                            + ", 12/31 23:00 "
                            + o23
    ...
    public void Test4176686() {
        ...
        String[] DATA = {
            ...
            z1.getDisplayName(false, TimeZone.SHORT),
            "GMT+1:30",
            "getDisplayName(false, LONG)/std zone",
            z1.getDisplayName(false, TimeZone.LONG),
            "GMT+01:30",
            "getDisplayName(true, SHORT)/std zone",
            z1.getDisplayName(true, TimeZone.SHORT),
            "GMT+1:30",
            "getDisplayName(true, LONG)/std zone",
            z1.getDisplayName(true, TimeZone.LONG),
            "GMT+01:30",
            "getDisplayName(false, SHORT)/dst zone",
            z2.getDisplayName(false, TimeZone.SHORT),
            "GMT+1:30",
            "getDisplayName(false, LONG)/dst zone",
            z2.getDisplayName(false, TimeZone.LONG),
            "GMT+01:30",
            "getDisplayName(true, SHORT)/dst zone",
            z2.getDisplayName(true, TimeZone.SHORT),
            "GMT+2:15",
            "getDisplayName(true, LONG)/dst zone",
            z2.getDisplayName(true, TimeZone.LONG),
            "GMT+02:15",
            "DateFormat.format(std)/std zone",
            fmt1.format(std),
            "GMT+1:30",
            "DateFormat.format(dst)/std zone",
            fmt1.format(dst),
            "GMT+1:30",
            "DateFormat.format(std)/dst zone",
            fmt2.format(std),
            "GMT+1:30",
            "DateFormat.format(dst)/dst zone",
            fmt2.format(dst),
            "GMT+2:15",
        };
    ...
    /**
     * SimpleTimeZone.getOffset() throws IllegalArgumentException when to get of 2/29/1996 (leap
     * day).
     */
    @Test
    public void Test4208960() {
        ...
        try {
            /*int offset =*/ tz.getOffset(
                    GregorianCalendar.AD, 1996, Calendar.FEBRUARY, 29, Calendar.THURSDAY, 0);
            // offset = 0;
        } catch (IllegalArgumentException e) {
        ...
        try {
            /*int offset =*/ tz.getOffset(
                    GregorianCalendar.AD, 1997, Calendar.FEBRUARY, 29, Calendar.THURSDAY, 0);
            // offset = 0;
    ...
    /**
     * Test to see if DateFormat understands zone equivalency groups. It might seem that this should
     * be a DateFormat test, but it's really a TimeZone test -- the changes to DateFormat are minor.
     *
     * <p>We use two known, zones that are equivalent, where one zone has localized name data, and
     * the other doesn't, in some locale.
     */
    @Test
    ...
    public void TestJ5134() {
        ...
        while (time < endTime) {
            ...
            if (offset != jdkOffset) {
                errln(
                        "FAIL: Incompatible UTC offset -> JDK:"
                                + jdkOffset
                                + "/ICU:"
                                + offset
                                + " ["
    ...
    public void TestT5280() {
        ...
        for (int i = 0; i < tzids.length; i++) {
            ...
            if (offset != newRawOffset) {
                if (isJdkZone) {
                    // JDK TimeZone#setRawOffset() only update the last rule, and getRawOffset()
                    // returns
                    // the current raw offset. Therefore, they might be different.
                    logln(
                            "Modified zone("
                                    + tz.getID()
                                    + ") - getRawOffset returns "
                                    + offset
                                    + "/ Expected: "
                                    + newRawOffset);
                } else {
            ...
            if (useDst != newDst) {
                errln(
                        "FAIL: Modified zone("
                                + tz.getID()
                                + ") - useDaylightTime has changed from "
                                + useDst
                                + " to "
                                + newDst);
            }
    ...
    public void TestJohannesburg() {
        ...
        int offset =
                johannesburg.getOffset(
                        GregorianCalendar.AD, 2007, Calendar.JULY, 5, Calendar.THURSDAY, 0);

        if (offset != expectedOffset) {
            errln(
                    "FAIL: zone "
                            + j_id
                            + " returned offset in July "
                            + offset
                            + ", expected "
                            + expectedOffset);
        } else {
        ...
        int offset2 =
                johannesburg.getOffset(
                        GregorianCalendar.AD, 2007, Calendar.DECEMBER, 12, Calendar.WEDNESDAY, 0);

        if (offset2 != expectedOffset) {
            errln(
                    "FAIL: zone "
                            + j_id
                            + " returned offset in December "
                            + offset2
                            + ", expected "
                            + expectedOffset);
        } else {
                            ...
                            new Runnable() {
                                ...
                                public void run() {
                                    for (int j = 0; j < 10000; j++) {
                                        try {
                                            com.ibm.icu.util.TimeZone.getTimeZone("GMT")
                                                    .getDisplayName();
                                        } catch (Exception e) {
    ...
    public void TestNegativeDaylightSaving() {
        ...
        if (offsets[0] != stdOff || offsets[1] != save) {
            errln(
                    "FAIL: Expected [stdoff="
                            + stdOff
                            + ",save="
                            + save
                            + "] on the test date (Jan 15), actual[stdoff="
                            + offsets[0]
                            + ",save="
                            + offsets[1]
                            + "]");
        ...
        if (offsets[0] != stdOff || offsets[1] != 0) {
            errln(
                    "FAIL: Expected [stdoff="
                            ...
                            + ",save="
                            + 0
                            + "] on the test date (Jul 15), actual[stdoff="
                            + offsets[0]
                            + ",save="
                            + offsets[1]
                            + "]");
