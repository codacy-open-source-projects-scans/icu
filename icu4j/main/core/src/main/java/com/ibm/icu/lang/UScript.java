 ...
 *
 * <p>The current set of script code constants supports at least all scripts that are encoded in the
 * version of Unicode which ICU currently supports. The names of the constants are usually derived
 * from the Unicode script property value aliases. See UAX #24 Unicode Script Property
 * (http://www.unicode.org/reports/tr24/) and
 * http://www.unicode.org/Public/UCD/latest/ucd/PropertyValueAliases.txt .
 *
 * <p>In addition, constants for many ISO 15924 script codes are included, for use with language
 * tags, CLDR data, and similar. Some of those codes are not used in the Unicode Character Database
 * (UCD). For example, there are no characters that have a UCD script property value of Hans or
 * Hant. All Han ideographs have the Hani script property value in Unicode.
 *
 * <p>Private-use codes Qaaa..Qabx are not included, except as used in the UCD or in CLDR.
 *
 * <p>Starting with ICU 55, script codes are only added when their scripts have been or will
 * certainly be encoded in Unicode, and have been assigned Unicode script property value aliases, to
 * ensure that their script names are stable and match the names of the constants. Script codes like
 * Latf and Aran that are not subject to separate encoding may be added at any time.
 *
 * @stable ICU 2.4
 */
public final class UScript {
    ...
    public static final int INHERITED =
            1; /* Zinh */ /* "Code for inherited script", for non-spacing combining marks; also Qaai */
     ...
     * @deprecated ICU 54
     */
    @Deprecated public static final int DUPLOYAN_SHORTAND = DUPLOYAN;
     ...
     * Returns MALAYAM given "Malayam" OR "Mlym". Returns LATIN given "en" OR "en_US"
     *
     * @param locale Locale
     ...
     * Returns MALAYAM given "Malayam" OR "Mlym". Returns LATIN given "en" OR "en_US"
     *
     * <p>Note: To search by short or long script alias only, use {@link #getCodeFromName(String)}
    ...
    public static final int[] getCode(String nameOrAbbrOrLocale) {
        ...
        if (lastSepPos < 0
            ...
            int propNum =
                    UCharacter.getPropertyValueEnumNoThrow(UProperty.SCRIPT, nameOrAbbrOrLocale);
    ...
    /**
     * Returns the script code associated with the given Unicode script property alias (name or
     * abbreviation). Short aliases are ISO 15924 script codes. Returns MALAYAM given "Malayam" OR
     * "Mlym".
     *
    ...
    /**
     * Gets the script code associated with the given codepoint. Returns UScript.MALAYAM given
     * 0x0D02
     *
    ...
    public static final int getScript(int codepoint) {
        if (codepoint >= UCharacter.MIN_VALUE & codepoint <= UCharacter.MAX_VALUE) {
            int scriptX =
                    UCharacterProperty.INSTANCE.getAdditional(codepoint, 0)
                            & UCharacterProperty.SCRIPT_X_MASK;
    ...
    /**
     * Do the Script_Extensions of code point c contain script sc? If c does not have explicit
     * Script_Extensions, then this tests whether c has the Script property value sc.
     *
     * <p>Some characters are commonly used in multiple scripts. For more information, see UAX #24:
    ...
    public static final boolean hasScript(int c, int sc) {
        int scriptX =
                UCharacterProperty.INSTANCE.getAdditional(c, 0) & UCharacterProperty.SCRIPT_X_MASK;
        int codeOrIndex = scriptX & UCharacterProperty.MAX_SCRIPT;
        if (scriptX < UCharacterProperty.SCRIPT_X_WITH_COMMON) {
     ...
     *
     * <ul>
     *   <li>If c does have Script_Extensions, then the return value is the negative number of
     *       Script_Extensions codes (= -set.cardinality()); in this case, the Script property value
     *       (normally Common or Inherited) is not included in the set.
     *   <li>If c does not have Script_Extensions, then the one Script code is put into the set and
     *       also returned.
     *   <li>If c is not a valid code point, then the one {@link #UNKNOWN} code is put into the set
     *       and also returned.
     * </ul>
     *
     * In other words, if the return value is non-negative, it is c's single Script code and the set
     * contains exactly this Script code. If the return value is -n, then the set contains c's
     * n&gt;=2 Script_Extensions script codes.
     *
     * <p>Some characters are commonly used in multiple scripts. For more information, see UAX #24:
     * http://www.unicode.org/reports/tr24/.
     *
     * @param c code point
     * @param set set of script code integers; will be cleared, then bits are set corresponding to
     *     c's Script_Extensions
     * @return negative number of script codes in c's Script_Extensions, or the non-negative single
     *     Script value
     * @stable ICU 49
     */
    ...
    /**
     * Returns the long Unicode script name, if there is one. Otherwise returns the 4-letter ISO
     * 15924 script code. Returns "Malayam" given MALAYALAM.
     *
    ...
    public static final String getName(int scriptCode) {
        return UCharacter.getPropertyValueName(
                UProperty.SCRIPT, scriptCode, UProperty.NameChoice.LONG);
    }

    /**
     * Returns the 4-letter ISO 15924 script code, which is the same as the short Unicode script
     * name if Unicode has names for the script. Returns "Mlym" given MALAYALAM.
     *
     * @param scriptCode int script code
    ...
    public static final String getShortName(int scriptCode) {
        return UCharacter.getPropertyValueName(
                UProperty.SCRIPT, scriptCode, UProperty.NameChoice.SHORT);
    }

    /**
     * Script metadata (script properties). See
    ...
    /**
     * Script usage constants. See UAX #31 Unicode Identifier and Pattern Syntax.
    ...
    /**
     * Returns the script sample character string. This string normally consists of one code point
     * but might be longer. The string is empty if the script is not encoded.
     *
    ...
    /**
     * Returns the script usage according to UAX #31 Unicode Identifier and Pattern Syntax. Returns
     * {@link ScriptUsage#NOT_ENCODED} if the script is not encoded in Unicode.
     *
     * @param script script code
     * @return script usage
     * @see ScriptUsage
    ...
    /**
     * Returns true if the script allows line breaks between letters (excluding hyphenation). Such a
     * script typically requires dictionary-based line breaking. For example, Hani and Thai.
     *
     * @param script script code
    ...
    /**
     * Returns true if in modern (or most recent) usage of the script case distinctions are
     * customary. For example, Latn and Cyrl.
     *
     * @param script script code
