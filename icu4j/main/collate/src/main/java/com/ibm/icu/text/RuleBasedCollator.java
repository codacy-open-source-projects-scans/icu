...
/**
 * RuleBasedCollator is a concrete subclass of Collator. It allows customization of the Collator via
 * user-specified rule sets. RuleBasedCollator is designed to be fully compliant to the <a
 * href="https://www.unicode.org/reports/tr10/">Unicode Collation Algorithm (UCA)</a> and conforms
 * to ISO 14651.
 *
 * <p>A Collator is thread-safe only when frozen. See {@link #isFrozen()} and {@link
 * com.ibm.icu.util.Freezable}.
 *
 * <p>Users are strongly encouraged to read the <a
 * href="https://unicode-org.github.io/icu/userguide/collation">User Guide</a> for more information
 * about the collation service before using this class.
 *
 * <p>Create a RuleBasedCollator from a locale by calling the getInstance(Locale) factory method in
 * the base class Collator. Collator.getInstance(Locale) creates a RuleBasedCollator object based on
 * the collation rules defined by the argument locale. If a customized collation ordering or
 * attributes is required, use the RuleBasedCollator(String) constructor with the appropriate rules.
 * The customized RuleBasedCollator will base its ordering on the CLDR root collation, while
 * re-adjusting the attributes and orders of the characters in the specified rule accordingly.
 *
 * <p>RuleBasedCollator provides correct collation orders for most locales supported in ICU. If
 * specific data for a locale is not available, the orders eventually falls back to the <a
 * href="http://www.unicode.org/reports/tr35/tr35-collation.html#Root_Collation">CLDR root sort
 * order</a>.
 *
 * <p>For information about the collation rule syntax and details about customization, please refer
 * to the <a href="https://unicode-org.github.io/icu/userguide/collation/customization">Collation
 * customization</a> section of the User Guide.
 *
 * <p><strong>Note</strong> that there are some differences between the Collation rule syntax used
 * in Java and ICU4J:
 *
 * <ul>
 *   <li>According to the JDK documentation: <br>
 *       <i>Modifier '!' : Turns on Thai/Lao vowel-consonant swapping. If this rule is in force when
 *       a Thai vowel of the range &#92;U0E40-&#92;U0E44 precedes a Thai consonant of the range
 *       &#92;U0E01-&#92;U0E2E OR a Lao vowel of the range &#92;U0EC0-&#92;U0EC4 precedes a Lao
 *       consonant of the range &#92;U0E81-&#92;U0EAE then the vowel is placed after the consonant
 *       for collation purposes. <br>
 *       If a rule is without the modifier '!', the Thai/Lao vowel-consonant swapping is not turned
 *       on. </i> <br>
 *       ICU4J's RuleBasedCollator does not support turning off the Thai/Lao vowel-consonant
 *       swapping, since the UCA clearly states that it has to be supported to ensure a correct
 *       sorting order. If a '!' is encountered, it is ignored.
 *   <li>As mentioned in the documentation of the base class Collator, compatibility decomposition
 *       mode is not supported.
 * </ul>
 *
 * <p><strong>Examples</strong>
 *
 * <p>Creating Customized RuleBasedCollators:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * Making changes to an existing RuleBasedCollator to create a new <code>Collator</code> object, by
 * appending changes to the existing rule:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * Putting in a new primary ordering before the default setting, e.g. sort English characters before
 * or after Japanese characters in the Japanese <code>Collator</code>:
 *
 * <blockquote>
 *
 * <pre>
...
public final class RuleBasedCollator extends Collator implements Cloneable {
    ...
    /**
     * Constructor that takes the argument rules for customization. The collator will be based on
     * the CLDR root collation, with the attributes and re-ordering of the characters specified in
     * the argument rules.
     *
     * <p>See the User Guide's section on <a
     * href="https://unicode-org.github.io/icu/userguide/collation/customization">Collation
    ...
    private final void internalBuildTailoring(String rules) throws Exception {
        ...
        try {
            ...
            Object builder =
                    builderClass.getConstructor(CollationTailoring.class).newInstance(base);
        ...
        synchronized (tailoring) {
            if (tailoring.maxExpansions == null) {
                tailoring.maxExpansions =
                        CollationElementIterator.computeMaxExpansions(tailoring.data);
            }
        }
    }
    ...
    /**
     * Return a CollationElementIterator for the given CharacterIterator. The source iterator's
     * integrity will be preserved since a new copy will be created for use.
     *
     * @see CollationElementIterator
     * @stable ICU 2.8
     */
    ...
    /**
     * Return a CollationElementIterator for the given UCharacterIterator. The source iterator's
     * integrity will be preserved since a new copy will be created for use.
     *
     * @see CollationElementIterator
     * @stable ICU 2.8
    ...
    /**
     * Sets the Hiragana Quaternary mode to be on or off. When the Hiragana Quaternary mode is
     * turned on, the collator positions Hiragana characters before all non-ignorable characters in
     * QUATERNARY strength. This is to produce a correct JIS collation order, distinguishing between
     * Katakana and Hiragana characters.
     *
     * <p>This attribute was an implementation detail of the CLDR Japanese tailoring. Since ICU 50,
     * this attribute is not settable any more via API functions. Since CLDR 25/ICU 53, explicit
     * quaternary relations are used to achieve the same Japanese sort order.
     *
     * @param flag true if Hiragana Quaternary mode is to be on, false otherwise
     * @see #setHiraganaQuaternaryDefault
     * @see #isHiraganaQuaternary
     * @deprecated ICU 50 Implementation detail, cannot be set via API, was removed from
     *     implementation.
     */
    @Deprecated
    ...
    /**
     * Sets the Hiragana Quaternary mode to the initial mode set during construction of the
     * RuleBasedCollator. See setHiraganaQuaternary(boolean) for more details.
     *
     * <p>This attribute was an implementation detail of the CLDR Japanese tailoring. Since ICU 50,
     * this attribute is not settable any more via API functions. Since CLDR 25/ICU 53, explicit
     * quaternary relations are used to achieve the same Japanese sort order.
    ...
    /**
     * Sets whether uppercase characters sort before lowercase characters or vice versa, in strength
     * TERTIARY. If false, lowercase characters sort before uppercase characters. If true, sort
     * upper case characters first. The default setting in a Collator object depends on the locale
     * data loaded from the resources. For most locales, the default is false, but for others, such
     * as "da" or "mt", the default could be true.
     *
     * @param upperfirst true to sort uppercase characters before lowercase characters, false to
     *     sort lowercase characters before uppercase characters
    ...
    /**
     * Sets the orders of lower cased characters to sort before upper cased characters, in strength
     * TERTIARY. If true is set, the RuleBasedCollator will sort lower cased characters before the
     * upper cased ones. Otherwise, if false is set, the RuleBasedCollator will ignore case
     * preferences. The default default setting in a Collator object depends on the locale data
     * loaded from the resources.
     *
     * @param lowerfirst true for sorting lower cased characters before upper cased characters,
     *     false to ignore case preferences.
     * @see #isLowerCaseFirst
     * @see #isUpperCaseFirst
    ...
    /**
     * Sets the case first mode to the initial mode set during construction of the
     * RuleBasedCollator. See setUpperCaseFirst(boolean) and setLowerCaseFirst(boolean) for more
     * details.
     *
     * @see #isLowerCaseFirst
     * @see #isUpperCaseFirst
    ...
    /**
     * Sets the alternate handling mode to the initial mode set during construction of the
     * RuleBasedCollator. See setAlternateHandling(boolean) for more details.
     *
    ...
    /**
     * Sets the case level mode to the initial mode set during construction of the
     * RuleBasedCollator. See setCaseLevel(boolean) for more details.
     *
    ...
    /**
     * Sets the decomposition mode to the initial mode set during construction of the
     * RuleBasedCollator. See setDecomposition(int) for more details.
     *
    ...
    /**
     * Sets the French collation mode to the initial mode set during construction of the
     * RuleBasedCollator. See setFrenchCollation(boolean) for more details.
     *
    ...
    /**
     * Sets the collation strength to the initial mode set during the construction of the
     * RuleBasedCollator. See setStrength(int) for more details.
     *
     * @see #setStrength(int)
     * @see #getStrength
     * @stable ICU 2.8
    ...
    /**
     * Sets the mode for the direction of SECONDARY weights to be used in French collation. If set
     * to false, which treats SECONDARY weights in the order they appear. If set to true, the
     * SECONDARY weights will be sorted backwards. See the section on <a
     * href="https://unicode-org.github.io/icu/userguide/collation/architecture">French
     * collation</a> for more information. The default setting in a Collator object depends on the
     ...
     * See the UCA definition on <a
     * href="https://www.unicode.org/reports/tr10/#Variable_Weighting">Variable Weighting</a>. This
     * attribute will only be effective when QUATERNARY strength is set. If the mode is set to
     * false, it corresponds to the NON_IGNORABLE mode in UCA. In the NON_IGNORABLE mode, the
     * RuleBasedCollator treats all the code points with non-ignorable primary weights in the same
     * way. If the mode is set to true, the behavior corresponds to SHIFTED defined in UCA, this
     * causes code points with PRIMARY orders that are equal or below the variable top value to be
     * ignored in PRIMARY order and moved to the QUATERNARY order. The default setting in a Collator
     * object depends on the locale data loaded from the resources. For most locales, the default is
     * false, but for others, such as "th", the default could be true.
     *
     * @param shifted true if SHIFTED behavior for alternate handling is desired, false for the
     *     NON_IGNORABLE behavior.
     * @see #isAlternateHandlingShifted
    ...
    /**
     * When case level is set to true, an additional weight is formed between the SECONDARY and
     * TERTIARY weight, known as the case level. The case level is used to distinguish large and
     * small Japanese Kana characters. Case level could also be used in other situations. For
     * example to distinguish certain Pinyin characters. If the value is false, it means the case
     * level is not generated. The contents of the case level are affected by the case first mode. A
     * simple way to ignore accent differences in a string is to set the strength to PRIMARY and
     * enable case level. The default setting in a Collator object depends on the locale data loaded
     * from the resources.
     *
     * <p>See the section on <a
     * href="https://unicode-org.github.io/icu/userguide/collation/architecture">case level</a> for
     * more information.
     *
     ...
     *
     * <p>The default decompositon mode for the Collator is NO_DECOMPOSITON, unless specified
     ...
     * @see #QUATERNARY
     * @see #IDENTICAL
     * @exception IllegalArgumentException If the new strength value is not one of PRIMARY,
     *     SECONDARY, TERTIARY, QUATERNARY or IDENTICAL.
     * @stable ICU 2.8
     */
    @Override
     ...
     * behavior. If that attribute is set to NON_IGNORABLE, then the variable top has no effect.
     *
    ...
    public RuleBasedCollator setMaxVariable(int group) {
        ...
        } else if (Collator.ReorderCodes.FIRST <= group
                && group <= Collator.ReorderCodes.CURRENCY) {
    ...
    private void internalSetVariableTop(long varTop) {
        if (varTop != settings.readOnly().variableTop) {
            ...
            if (group < Collator.ReorderCodes.FIRST || Collator.ReorderCodes.CURRENCY < group) {
                throw new IllegalArgumentException(
                        "The variable top must be a primary weight in "
            ...
            if (varTop != settings.readOnly().variableTop) {
                CollationSettings ownedSettings = getOwnedSettings();
                ownedSettings.setMaxVariable(
                        group - Collator.ReorderCodes.FIRST, getDefaultSettings().options);
                ownedSettings.variableTop = varTop;
                setFastLatinOptions(ownedSettings);
            }
        }
    ...
    /**
     * {@icu} When numeric collation is turned on, this Collator makes substrings of digits sort
     * according to their numeric values.
     *
     * <p>This is a way to get '100' to sort AFTER '2'. Note that the longest digit substring that
     * can be treated as a single unit is 254 digits (not counting leading zeros). If a digit
     * substring is longer than that, the digits beyond the limit will be treated as a separate
     * digit substring.
     *
     * <p>A "digit" in this sense is a code point with General_Category=Nd, which does not include
     * circled numbers, roman numerals, etc. Only a contiguous digit substring is considered, that
     * is, non-negative integers without separators. There is no support for plus/minus signs,
     * decimals, exponents, etc.
     *
     ...
     * @throws IllegalArgumentException if the reordering codes are malformed in any way (e.g.
     *     duplicates, multiple reset codes, overlapping equivalent scripts)
     * @see #getReorderCodes
    ...
    /**
     * Gets the collation tailoring rules for this RuleBasedCollator. Equivalent to String
     * getRules(false).
     *
    ...
    /**
     * Returns current rules. The argument defines whether full rules (root collation + tailored)
     * rules are returned or just the tailoring.
     *
     * <p>The root collation rules are an <i>approximation</i> of the root collator's sort order.
     * They are almost never used or useful at runtime and can be removed from the data. See <a
     * href="https://unicode-org.github.io/icu/userguide/collation/customization#building-on-existing-locales">User
     * Guide: Collation Customization, Building on Existing Locales</a>
     *
     * <p>{@link #getRules()} should normally be used instead.
     *
     * @param fullrules true if the rules that defines the full set of collation order is required,
     *     otherwise false for returning only the tailored rules
     ...
     * @stable ICU 3.4
     */
    public void getContractionsAndExpansions(
            UnicodeSet contractions, UnicodeSet expansions, boolean addPrefixes) throws Exception {
    ...
    /**
     * Adds the contractions that start with character c to the set. Ignores prefixes. Used by
     * AlphabeticIndex.
     *
     * @internal
     ...
     *
     * <p>General recommendation: <br>
     * If comparison are to be done to the same String multiple times, it would be more efficient to
     * generate CollationKeys for the Strings and use CollationKey.compareTo(CollationKey) for the
     * comparisons. If the each Strings are compared to only once, using the method
     * RuleBasedCollator.compare(String, String) will have a better performance.
     *
     * <p>See the class documentation for an explanation about CollationKeys.
     *
     * @param source the text String to be transformed into a collation key.
     * @return the CollationKey for the given String based on this RuleBasedCollator's collation
    ...
    /**
     * Gets the simpler form of a CollationKey for the String source following the rules of this
     * Collator and stores the result into the user provided argument key. If key has a internal
     * byte array of length that's too small for the result, the internal byte array will be grown
    ...
    private RawCollationKey getRawCollationKey(
            CharSequence source, RawCollationKey key, CollationBuffer buffer) {
        if (key == null) {
    ...
    private void writeSortKey(CharSequence s, CollationKeyByteSink sink, CollationBuffer buffer) {
        ...
        if (settings.readOnly().dontCheckFCD()) {
            ...
            CollationKeys.writeSortKeyUpToQuaternary(
                    buffer.leftUTF16CollIter,
                    data.compressibleBytes,
                    settings.readOnly(),
                    sink,
                    Collation.PRIMARY_LEVEL,
                    CollationKeys.SIMPLE_LEVEL_FALLBACK,
                    true);
        } else {
            buffer.leftFCDUTF16Iter.setText(numeric, s, 0);
            CollationKeys.writeSortKeyUpToQuaternary(
                    buffer.leftFCDUTF16Iter,
                    data.compressibleBytes,
                    settings.readOnly(),
                    sink,
                    Collation.PRIMARY_LEVEL,
    ...
    private void writeIdenticalLevel(CharSequence s, CollationKeyByteSink sink) {
        ...
        // Sync the key with the buffer again which got bytes appended and may have been
        // reallocated.
     ...
     * level of difference considered significant.
     *
     * <p>{@icunote} This can return QUATERNARY strength, which is not supported by the JDK version.
     *
     ...
     * @see #isLowerCaseFirst
     * @see #setCaseFirstDefault
     * @return true if upper cased characters are sorted before lower cased characters, false
     *     otherwise
     * @stable ICU 2.8
     */
    ...
    /**
     * Checks if the alternate handling behavior is the UCA defined SHIFTED or NON_IGNORABLE. If
     * return value is true, then the alternate handling attribute for the Collator is SHIFTED.
     * Otherwise if return value is false, then the alternate handling attribute for the Collator is
    ...
    /**
     * Checks if the Hiragana Quaternary mode is set on. See setHiraganaQuaternary(boolean) for more
     * details.
     *
     * <p>This attribute was an implementation detail of the CLDR Japanese tailoring. Since ICU 50,
     * this attribute is not settable any more via API functions. Since CLDR 25/ICU 53, explicit
    ...
    /**
     * Method to retrieve the numeric collation value. When numeric collation is turned on, this
     * Collator generates a collation key for the numeric value of substrings of digits. This is a
     * way to get '100' to sort AFTER '2'
     *
     * @see #setNumericCollation
     * @see #setNumericCollationDefault
    ...
    public boolean equals(Object obj) {
        ...
        if ((thisIsRoot || theseRules.length() != 0) && (otherIsRoot || otherRules.length() != 0)) {
    ...
    /**
     * Compares the source text String to the target text String according to the collation rules,
     * strength and decomposition mode for this RuleBasedCollator. Returns an integer less than,
     * equal to or greater than zero depending on whether the source String is less than, equal to
     * or greater than the target String. See the Collator class description for an example of use.
     *
     * <p>General recommendation: <br>
     * If comparison are to be done to the same String multiple times, it would be more efficient to
     * generate CollationKeys for the Strings and use CollationKey.compareTo(CollationKey) for the
     * comparisons. If speed performance is critical and object instantiation is to be reduced,
     * further optimization may be achieved by generating a simpler key of the form RawCollationKey
     * and reusing this RawCollationKey object with the method RuleBasedCollator.getRawCollationKey.
     * Internal byte representation can be directly accessed via RawCollationKey and stored for
     * future use. Like CollationKey, RawCollationKey provides a method RawCollationKey.compareTo
     * for key comparisons. If the each Strings are compared to only once, using the method
     * RuleBasedCollator.compare(String, String) will have a better performance.
     *
    ...
    /**
     * Abstract iterator for identical-level string comparisons. Returns FCD code points and handles
     * temporary switching to NFD.
     *
     * <p>As with CollationIterator, Java NFDIterator instances are partially constructed and
     * cached, and completed when reset for use. C++ NFDIterator instances are stack-allocated.
     */
    private abstract static class NFDIterator {
        ...
        /**
         * Returns the next code point from the internal normalization buffer, or else the next text
         * code point. Returns -1 at the end of the text.
         */
         ...
         * @return the first code point in c's decomposition, or c itself if it was decomposed
         *     already or if it does not decompose
         */
    ...
    private static final int compareNFDIter(
        ...
        try {
            ...
            if (roSettings.dontCheckFCD()) {
                ...
                return compareNFDIter(
                        nfcImpl, buffer.leftFCDUTF16NFDIter, buffer.rightFCDUTF16NFDIter);
            }
        } finally {
            releaseCollationBuffer(buffer);
        }
