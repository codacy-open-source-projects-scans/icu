...
public class ResourceFallbackCodeGenerator implements CodeGenerator {
    ...
    public void generateCode(Path cldrPath, PrintWriter cFileOut, PrintWriter javaFileOut) {
        ...
        supplementalData.accept(
                CldrData.PathOrder.NESTED_GROUPING,
                new CldrData.PrefixVisitor() {
                    @Override
    ...
    private void handleLikelySubtag(CldrValue value) {
        ...
        if (toPieces.length != 3) {
            throw new IllegalArgumentException(
                    "Didn't get 3 segments in 'to' value: from=" + from + ", to=" + to);
        }
        ...
        if (fromPieces.length >= 3) {
            throw new IllegalArgumentException(
                    "'from' entry has a non-'und' language and also has a script code: from="
                            + from
                            + ", to="
                            + to);
        }
        if (fromPieces.length == 2 && fromPieces[1].length() > 3) {
            // the locale ID consists of just a language and a script-- the script code is redundant
            // and doesn't
        ...
        String defaultScript =
                toPieces[1]; // toPieces is always three elements, so the second one is always the
        // script
    ...
    private void generateCFile(PrintWriter out) {
        ...
        Map<String, Integer> scriptIndex =
                buildCompositeString(defaultScripts.values(), "scriptCodeChars", out);
        Map<String, Integer> localeIDIndex =
                buildCompositeString(defaultScripts.keySet(), "dsLocaleIDChars", out);
        writeStringToStringIndex(
                defaultScripts, localeIDIndex, scriptIndex, "defaultScriptTable", out);
        ...
        writeStringToStringIndex(
                parentLocales, localeIDIndex, localeIDIndex, "parentLocaleTable", out);

        out.println();
        out.println("#endif  // INCLUDED_FROM_URESBUND_CPP");
    }

    private Map<String, Integer> buildCompositeString(
            Collection<String> strings, String variableName, PrintWriter out) {
    ...
    private void writeStringToStringIndex(
